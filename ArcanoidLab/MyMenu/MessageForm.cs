...
namespace ArcanoidLab
{
  ...
  public class MessageForm
  {
    ...
    public MessageForm(VideoMode mode, string textLabel, string textButtonYes, string textButtonNo, Game game, string textAlias)
    {
      ...
      // кнопки на форме
      ButtonMenus.Add(new ButtonMenu(70, 30, textButtonYes, textAlias, 16, "FreeMonospacedBold", (mode.Width / 2) - (50 / 2), (mode.Height / 2) - 40, Color.Red, Color.Yellow, mode));
    ...
    private void KeyHandler(RenderTarget window)
    {
      ...
      for (int i = 0; i < ButtonMenus.Count; i++)
      {
        localBounds = ButtonMenus[i].MenuItemRect.GetLocalBounds();
        globalBounds = ButtonMenus[i].MenuItemRect.Transform.TransformRect(localBounds);
        ...
        if (globalBounds.Contains(worldMouseCoords.X, worldMouseCoords.Y))
        {
          ...
          else if (Mouse.IsButtonPressed(Mouse.Button.Left) && ButtonMenus[i].AliasButton == "newGame")
          {
